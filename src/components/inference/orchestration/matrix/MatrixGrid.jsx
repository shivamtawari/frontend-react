...
export default function MatrixGrid({
  ...
  const TREE_ORIGIN = 16;
  const TREE_INDENT = 22;
  const LABEL_DOT_SIZE = 10;
  ...
  const renderHierarchyGuides = (label) => {
    ...
    for (let level = 0; level < label.depth - 1; level += 1) {
      guides.push(
        <span
          ...
          className="absolute top-0 bottom-0 z-0 w-px bg-ln2 dark:bg-slate-700/60"
    ...
    if (label.depth > 0) {
      ...
      guides.push(
        <span
          key="current-rail"
          className="absolute top-0 z-0 w-px bg-ln2 dark:bg-slate-700/60"
          style={{
      ...
      guides.push(
        <span
          key="branch-elbow"
          className="absolute z-0 h-px bg-ln2 dark:bg-slate-700/60"
          style={{
            left: `${parentRailLeft}px`,
            top: "50%",
    ...
    if (label.hasChildren) {
      guides.push(
        <span
          key="child-rail"
          className="absolute bottom-0 z-0 w-px bg-ln2 dark:bg-slate-700/60"
          style={{
      ...
      <div className="min-w-[860px] md:min-w-0">
        ...
        {displayLabels.map((l) => {
          ...
          return (
            <div
              key={l.id}
              className={`group grid grid-cols-1 md:grid-cols-5 gap-3.5 px-3 py-1.5 rounded-2xl transition-colors hover:bg-well/40 items-center my-0.5 ${
            ...
            >
              ...
              <div
                className="relative min-h-[46px] min-w-0"
                style={{
                  paddingLeft: `${
              ...
              >
                ...
                <div className="relative z-10 flex min-h-[46px] min-w-0 flex-col justify-center">
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`shrink-0 rounded-full ring-1 ring-black/10 dark:ring-white/10 ${
                        l.depth === 0 ? "h-3 w-3 shadow-xs" : "h-2.5 w-2.5"
                      }`}
                      style={{ backgroundColor: dotColor }}
                    />
                    <span className="truncate text-xs font-semibold text-t1">
                      {l.name}
                    </span>
                  </div>
                  <p
                    className="relative z-10 mt-0.5 truncate pl-4.5 text-[10px] text-t3"
                  >
                    {levelText}
                  </p>
                </div>
